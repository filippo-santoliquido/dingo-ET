...
class Result(DingoDataset):
    ...
    def get_all_injection_credible_levels(
        ...
        if self.metadata['injection_parameters'] is None:
            raise (
                TypeError,
        ...
        credible_levels = {
            ...
            for key in keys
            if isinstance(self.metadata['injection_parameters'].get(key, None), float)
        }
    ...
    def get_injection_credible_level(self, parameter: str, weighted: bool = False):
        ...
        if parameter in theta and parameter in self.metadata['injection_parameters']:
            credible_level = sum(
                np.array(theta[parameter].values < self.metadata['injection_parameters'][parameter])
                * weights
            ) / (sum(weights))
